# Modified version of inspyred/ec.py that allows for Sobol sampling and evolution of
# PSO metaparameters
...
class Individual(object):
    ...
    def __repr__(self):
        return "<Individual: candidate = {0}, fitness = {1}, birthdate = {2}>".format(
...
class EvolutionaryComputation(object):
    ...
    def _should_terminate(self, pop, ng, ne):
        ...
        if isinstance(self.terminator, collections.Iterable):
            for clause in self.terminator:
                self.logger.debug(
                    "termination test using {0} at generation {1} and evaluation {2}".format(
                        clause.__name__, ng, ne
                    )
                )
                terminate = terminate or clause(
                    population=pop,
                    num_generations=ng,
                    num_evaluations=ne,
                    args=self._kwargs,
            ...
            self.logger.debug(
                "termination test using {0} at generation {1} and evaluation {2}".format(
                    self.terminator.__name__, ng, ne
                )
            )
        ...
        if terminate:
            ...
            self.logger.debug(
                "termination from {0} at generation {1} and evaluation {2}".format(
                    self.termination_cause, ng, ne
                )
            )
        return terminate
    ...
    def evolve(
        ...
        **args
        ...
        self._kwargs["opt_args"]["weights_archive"] = {}

        for obj in self._kwargs["opt_args"]["weights_archive"]:
            self._kwargs["opt_args"]["weights_archive"][obj] = self._kwargs["opt_args"]["weights"][obj]

        flag_adaptive = 0
        if "gaussian_stdev" in self._kwargs:
            flag_adaptive = 1
            gaussian_stdev_base = self._kwargs["gaussian_stdev"].copy()
        ...
        if "initial_fit" not in self._kwargs:
            from scipy.stats.qmc import Sobol
            i = 0
            self.logger.debug("generating initial population")
            parameters = self._kwargs["opt_args"]["parameters"]
            bound_lo = self._kwargs["opt_args"]["Bounder"]["lo"]
            bound_hi = self._kwargs["opt_args"]["Bounder"]["hi"]
            amps = bound_hi - bound_lo
            n_parameters = len(self._kwargs["opt_args"]["parameters"])
            sampler = Sobol(d=n_parameters, scramble=True, seed=self._kwargs["opt_args"]["seed_evo"])
            samples = sampler.random_base2(m=self._kwargs["opt_args"]["base2_power"])
            while i < num_generated:
                #cs = generator(random=self._random, args=self._kwargs)
                sample = samples[i]
                cs = []
                for ind_param in range(n_parameters):
                    amp = amps[ind_param]
                    cs.append(amp * sample[ind_param] + bound_lo[ind_param])

                initial_cs.append(cs)
                i += 1
                self.logger.debug("evaluating initial population")
            initial_fit, self._kwargs["opt_args"]["weights_archive"] = evaluator(
                candidates=initial_cs, args=self._kwargs
            )
        else:
            initial_cs = self._kwargs["initial_cs"]
            print("Loading Init")
            initial_fit, self._kwargs["opt_args"]["weights_archive"] = evaluator(
                candidates=initial_cs, args=self._kwargs
            )
        ...
        for cs, fit in zip(initial_cs, initial_fit):
            if fit is not None:
                ...
                self.logger.warning(
                    "excluding candidate {0} because fitness received as None".format(
                        cs
                    )
                )
        ...
        if isinstance(self.observer, collections.Iterable):
            for obs in self.observer:
                self.logger.debug(
                    "observation using {0} at generation {1} and evaluation {2}".format(
        ...
        while not self._should_terminate(
            list(self.population), self.num_generations, self.num_evaluations
        ):
            # Select individuals.
            self.logger.debug(
                "selection using {0} at generation {1} and evaluation {2}".format(
                    self.selector.__name__, self.num_generations, self.num_evaluations
                )
            )
            parents = self.selector(
                random=self._random, population=list(self.population), args=self._kwargs
            )
            ...
            if isinstance(self.variator, collections.Iterable):
                for op in self.variator:
                    self.logger.debug(
                        "variation using {0} at generation {1} and evaluation {2}".format(
                            op.__name__, self.num_generations, self.num_evaluations
                        )
                    )
                    offspring_cs = op(
                        random=self._random, candidates=offspring_cs, args=self._kwargs
                    )
            else:
                self.logger.debug(
                    "variation using {0} at generation {1} and evaluation {2}".format(
                        self.variator.__name__,
                        self.num_generations,
                        self.num_evaluations,
                    )
                )
                offspring_cs = self.variator(
                    random=self._random, candidates=offspring_cs, args=self._kwargs
                )
            ...
            # Evaluate offspring.
            self.logger.debug(
                "evaluation using {0} at generation {1} and evaluation {2}".format(
                    evaluator.__name__, self.num_generations, self.num_evaluations
                )
            )
            offspring_fit, self._kwargs["opt_args"]["weights_archive"] = evaluator(
            ...
            # Replace individuals.
            self.logger.debug(
                "replacement using {0} at generation {1} and evaluation {2}".format(
                    self.replacer.__name__, self.num_generations, self.num_evaluations
                )
            )
            self.population = self.replacer(
                random=self._random,
            ...
            # Migrate individuals.
            self.logger.debug(
                "migration using {0} at generation {1} and evaluation {2}".format(
                    self.migrator.__name__, self.num_generations, self.num_evaluations
                )
            )
            self.population = self.migrator(
                random=self._random, population=self.population, args=self._kwargs
            )
            self.logger.debug("population size is now {0}".format(len(self.population)))

            # Archive individuals.
            self.logger.debug(
                "archival using {0} at generation {1} and evaluation {2}".format(
            ...
            # Change mutation gaussian stdev
            if flag_adaptive:
                if self._kwargs["opt_args"]["gaussian_stdev"]["skip"] > 0:
                    if (self.num_generations + 1) % self._kwargs["opt_args"]["gaussian_stdev"]["skip"] == 0:
                        self._kwargs["gaussian_stdev"] *= self._kwargs["opt_args"]["gaussian_stdev"]["decay"]

                    if (self.num_generations + 1) % self._kwargs["opt_args"]["adaptive_weighting_skip"] == 0:
                        self._kwargs["gaussian_stdev"] = gaussian_stdev_base.copy()
    ...
    def evolve(
        ...
        **args
        ...
        return EvolutionaryComputation.evolve(
            self, generator, evaluator, pop_size, seeds, maximize, bounder, **args
        )
...
class EDA(EvolutionaryComputation):
    ...
    def _internal_variation(self, random, candidates, args):
        ...
        stdev = [
            sum([(x - m) ** 2 for x in g]) / float(len(g) - 1)
